...
class SurnameVectorizer(object):
    ...
    def vectorize(self, surname, vector_length=-1):
        ...
        indices.extend(
            self.char_vocab.lookup_token(token) for token in surname
        )
    ...
    def from_serializable(cls, contents):
        ...
        """
        char_vocab = SequenceVocabulary.from_serializable(
            contents["char_vocab"]
        )
...
class SurnameGenerationModel(nn.Module):
    def __init__(
        ...
        self.fc = nn.Linear(
            in_features=rnn_hidden_size, out_features=char_vocab_size
        )
...
def sample_from_model(
    ...
    begin_seq_index = torch.tensor(
        begin_seq_index, dtype=torch.int64
    ).unsqueeze(dim=1)
    indices = [begin_seq_index]
    nationality_indices = torch.tensor(
        nationalities, dtype=torch.int64
    ).unsqueeze(dim=0)
...
try:
    for epoch_index in range(args.num_epochs):
        ...
        sample1 = "{}->{}".format(
            vectorizer.nationality_vocab.lookup_index(nationalities[0]),
            sampled_surnames[0],
        )
        sample2 = "{}->{}".format(
            vectorizer.nationality_vocab.lookup_index(nationalities[1]),
            sampled_surnames[1],
        )
